...
import '../application/my_event_bookings_controller.dart';
import '../data/event_repository.dart';
import '../models/event_booking.dart';
import '../models/event_booking_summary.dart';

class MyEventBookingsScreen extends ConsumerStatefulWidget {
  const MyEventBookingsScreen({super.key});

  @override
  ConsumerState<MyEventBookingsScreen> createState() => _MyEventBookingsScreenState();
}

class _MyEventBookingsScreenState extends ConsumerState<MyEventBookingsScreen> {
  final _cancellingIds = <String>{};

  Future<void> _cancel(EventBookingSummary summary) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Annuler la réservation ?'),
        content: Text(
          '${summary.eventName}, ${summary.booking.quantity} billet${summary.booking.quantity > 1 ? 's' : ''}. '
    ...
    try {
      await ref.read(eventRepositoryProvider).cancelBooking(summary.booking.id);
      ref.invalidate(myEventBookingsProvider);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    final bookingsAsync = ref.watch(myEventBookingsProvider);
              ...
              child: Column(
                ...
                children: [
                  ...
                  AppButton(
                    ...
                    onPressed: () => ref.invalidate(myEventBookingsProvider),
                  ),
                ],
              ),
            ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: AppSpacing.md),
                      Text('Aucun billet pour le moment.', style: textTheme.titleMedium),
                      const SizedBox(height: AppSpacing.sm),
                      Text(
                        'Vos billets d\'événement apparaîtront ici.',
...
class _EventBookingCard extends StatelessWidget {
  const _EventBookingCard({required this.summary, required this.isCancelling, required this.onCancel});
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(summary.eventName, style: textTheme.titleSmall, overflow: TextOverflow.ellipsis),
              ),
              Container(
          ...
          Text(
            '${_formatDateTime(summary.eventStartDatetime)} · ${summary.ticketCategoryLabel} · '
