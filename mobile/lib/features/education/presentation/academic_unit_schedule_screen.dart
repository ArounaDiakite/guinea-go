...
/// A day/time timetable - grouped by day (only days with at least one
/// slot show up, in DayOfWeek order) rather than a literal 7-column
/// grid, which wouldn't fit a real phone width without horizontal
/// scrolling or cramming text past readability.
class AcademicUnitScheduleScreen extends ConsumerWidget {
  const AcademicUnitScheduleScreen({super.key, required this.institutionId, required this.academicUnitId});
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => context.push(
          '/hub/school/academic-units/$academicUnitId/schedule/timeslots/new',
          extra: institutionId,
        ),
              ...
              children: [
                for (final day in sortedDays) ...[
                  ...
                  for (final item in byDay[day]!) ...[
                    _ScheduleItemCard(institutionId: institutionId, academicUnitId: academicUnitId, item: item),
                    const SizedBox(height: AppSpacing.sm),
                  ],
                  const SizedBox(height: AppSpacing.md),
                ],
...
class _ScheduleItemCard extends StatelessWidget {
  const _ScheduleItemCard({required this.institutionId, required this.academicUnitId, required this.item});
  ...
  Widget build(BuildContext context) {
    ...
    return AppCard(
      onTap: () => context.push(
        '/hub/school/academic-units/$academicUnitId/schedule/timeslots/${item.id}/edit',
        extra: institutionId,
      ),
      child: Row(
        children: [
