...
- **`countries/`, `cities/`, `currencies`** : actifs, montés dans `main.py`. Lecture publique (GET), écriture réservée à `system_administrator` (POST/PUT/DELETE). `DELETE` est un soft-delete via `is_active: False` (pas de champ `is_deleted` sur ces collections) — pour `countries`, `GET /countries/` filtre déjà sur `is_active: True` donc un pays supprimé disparaît de la liste publique ; pour `cities`, `GET /cities/`/`GET /cities/{country_code}` ne filtrent pas `is_active`, un enregistrement supprimé y reste visible (incohérence pré-existante, pas corrigée). `Country.currency_id` référence un vrai document `Currency` (résolu depuis un `currency_code` fourni en entrée) ; `City.country_code` reste une clé texte simple vers `Country.code`, pas un `country_id`. Seedées au démarrage (`database/seed.py::run_seed`) : devises GNF/XOF/GHS, pays GN/SN/GH, quelques villes.
